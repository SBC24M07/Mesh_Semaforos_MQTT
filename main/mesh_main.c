...
void static recv_cb(mesh_addr_t *from, mesh_data_t *data)
{
	switch(data->data[0]){
		...
		case CMD_BUTTON_PRESSED:
        	...
        	if (data->data[7] == 1){
        		ESP_LOGW(MESH_TAG, "Button pressed detected on node: "
            	    	MACSTR, MAC2STR(data->data + 1));
            }
			break;
		
		case CMD_MOVEMENT_DETECTED:
			if (data->size != 8) {
            	ESP_LOGE(MESH_TAG, "Error in receiving raw mesh data: Unexpected size");
            	return;
        	}
        	...
        	if (data->data[7] == 1){
        		ESP_LOGW(MESH_TAG, "Movement detected on node: "
            	    	MACSTR, MAC2STR(data->data + 1));
            }
			break;
		...
		case CMD_TRAFFIC_LIGHT:
			...
			ESP_LOGI(MESH_TAG, "Recibido orden de semaforo");
			traffic_light_process(from, data->data, data->size);
			break;
...
{
    esp_err_t err;
    uint8_t num_traffic_lights = 0;
    uint8_t cicle_pos = 0;
    uint8_t color_pos = 0;
    
    uint8_t green_timer = 10;
    uint8_t yellow_timer = 3;
    uint8_t red_timer = 1;
    uint8_t timer = 0;
    
    mesh_data_t data;
    uint8_t message[3];  // Un byte para el comando y otro para el valor booleano
    ...
    while (true) {
		if (esp_mesh_is_root()){
        	message[0] = CMD_TRAFFIC_LIGHT;    	 // Comando que identifica el mensaje
        	message[1] = 1;  // Fijar semaforo: 0 para informar, 1 para fijar
        	message[2] = (color_pos == 0)? TRAFFIC_LIGHT_GREEN : (color_pos == 1)? TRAFFIC_LIGHT_YELLOW : TRAFFIC_LIGHT_RED;		// Estado del semaforo
        	timer = (color_pos == 0)? green_timer : (color_pos == 1)? yellow_timer : red_timer;
        	
        	// Llenar la estructura de datos para el envío
        	data.size = sizeof(message);
        	data.proto = MESH_PROTO_BIN;        // Indicar que el mensaje es binario
       	 	data.tos = MESH_TOS_P2P;           // Tipo de servicio: punto a punto
        	data.data = message;
			
        	// Enviar el mensaje aL siguiente semaforo en la lista
        	xSemaphoreTake(s_route_table_lock, portMAX_DELAY);
        	
        	num_traffic_lights = (s_route_table_size > 4)? 4 : s_route_table_size;
        	//num_traffic_lights = s_traffic_route_table_size;
        	
            err = esp_mesh_send(&s_route_table[cicle_pos], &data, MESH_DATA_P2P, NULL, 0);
            ESP_LOGI(MESH_TAG, "Estableciendo semaforo [%d] " MACSTR ": err=%d", cicle_pos, MAC2STR(s_route_table[cicle_pos].addr), err);
        	...
        	if (color_pos > 2){
				color_pos = 0;
				cicle_pos++;
        		if (cicle_pos >= num_traffic_lights)
        			cicle_pos = 0;
			}

        	// Esperar 5 segundos antes de enviar nuevamente
        	vTaskDelay(timer * 1000 / portTICK_PERIOD_MS);
        } else {
...
//Root da la tabla de direcciones a todos los nodos
...
esp_err_t esp_mesh_comm_mqtt_task_start(void)
{
    ...
    if (!is_comm_mqtt_task_started) {
        xTaskCreate(esp_mesh_mac_update, "mqtt task", 3072, NULL, 5, NULL);
        xTaskCreate(traffic_light_control, "traffic light control", 4096, NULL, 6, NULL);
